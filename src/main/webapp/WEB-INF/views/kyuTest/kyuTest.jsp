...
$(function(){
	...
	$(document).on("click", "#gatherReview", function () {
		 let target = {"regularGatherNo":"${"regularGatherNo"}"}
		console.log(target)
		$.ajax({
			
			url:"${pageContext.request.contextPath}/kyuTest/kyuTest/gr", //서버 요청주소
			type: "get",  //요청방식 (get,post, put,, delete, patch)
			dataType: "text", //서버가 응답(보내온)한 데이터 타입 (text, html, xml, json)
			data : JSON.stringify(target),
			contentType:'application/json;charset=utf-8',
			success: function(result){
					alert(1)
					location.href='kyuTest/kyuTest';
			},error: function(err){
				alert(err+"에러 발생 !! ");
			}
			});
...
<input type="button" value="모임후기조회" id="gatherReview" > 
...
<table align="center" border="0" cellpadding="5" cellspacing="2" width="100%" bordercolordark="white" bordercolorlight="black">
    ...
    <c:otherwise>
	<c:forEach items="${requestScope.pageList}" var="gr">
		    <tr onmouseover="this.style.background='#eaeaea'" onmouseout="this.style.background='white'">
		        <td bgcolor="">
		            <p align="center"><span style="font-size:9pt;">
		            ${gr.reviewRegisDate}</span></p>
		        </td>
		        <td bgcolor="">
					<p><span style="font-size:9pt;">
					  ${gr.writerUser.userName} /  R </b>
					</span></p>
		        </td>

		        <td bgcolor="">
		            <p align="center"><span style="font-size:9pt;">
		            ${gr.gatherReviewContent}</span></p>
		        </td>
		    </tr>
    </c:forEach>
	</c:otherwise>
