  ...
  <jsp:include page="/WEB-INF/views/layout/header.jsp" />
        ...
        <div class="search-list-top">
          ...
          <div class="search-list-main-filter">
            <ul class="search-list-main-filter-wrap">
              <li class="search-list-main-filter-item" onclick="location.href='${pageContext.request.contextPath}/main/regularGather'">모임검색</li>
              <li class="search-list-main-filter-item selected" onclick="location.href='${pageContext.request.contextPath}/main/searchUserIndivForRegular'">회원검색</li>
            </ul>
         			...
         			<c:forEach items="${userList.content }" var="user">
						...
						<c:if test="${state1 ne state2}">
                			...
                			<div class="search-member-list-result-item-member-name">
                  				<a href="${pageContext.request.contextPath}/userProfile/profileGather/${user.userNo }">${user.userNick }</a>
                			</div>
              			</article>
         			</c:forEach>
         		</c:when>
	...
	<c:set var="doneLoop" value="false"/>
		  ...
		  <c:if test="${(startPage-blockCount) > 0}"> <!-- (-2) > 0  -->
		      <a class="pagination-newer" href="${pageContext.request.contextPath}/main/searchUserIndivForRegular/${keyword}?nowPage=${startPage-1}">PREV</a>
		  </c:if>
		...
		<span class="pagination-inner"> 
		  ...
		  <c:if test="${not doneLoop}" >
		         <a class="${i==nowPage?'pagination-active':page}" href="${pageContext.request.contextPath}/main/searchUserIndivForRegular/${keyword}?nowPage=${i}">${i}</a> 
		  </c:if>
		 ...
		 <c:if test="${(startPage+blockCount)<=userList.getTotalPages()}">
		     <a class="pagination-older" href="${pageContext.request.contextPath}/main/searchUserIndivForRegular/${keyword}?nowPage=${startPage+blockCount}">NEXT</a>
		 </c:if> 
		</c:if>		 
