...
public interface InquiryRepository extends JpaRepository<Inquiry, Long>{
	...
	//마이페이지에서 쓰는
	@Query(value = "select i.inq_no, g.gather_name, i.inq_subject, i.inq_regis_date, r.inq_no as state  from (select * from inquiry where user_no =?1) i \r\n"
			+ "left outer join (select distinct inq_no from inquiry_reply) r on i.inq_no=r.inq_no join gather g on i.gather_no=g.gather_no", nativeQuery = true)
